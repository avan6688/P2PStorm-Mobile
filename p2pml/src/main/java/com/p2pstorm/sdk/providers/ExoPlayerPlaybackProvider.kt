    ...
    override suspend fun getPlaybackPositionAndSpeed(): PlaybackInfo =
        mutex.withLock {
            ...
            if (currentSegment == null) {
                // Fallback: find nearest segment instead of crashing
                val nearest = currentSegments.values.minByOrNull {
                    kotlin.math.abs(it.startTime - currentPlayback)
                }
                val fallbackTime = nearest?.let {
                    val dt = currentPlayback - it.startTime
                    it.absoluteStartTime + dt
                } ?: playbackPositionInSeconds
                return PlaybackInfo(fallbackTime, playbackSpeed)
            }
